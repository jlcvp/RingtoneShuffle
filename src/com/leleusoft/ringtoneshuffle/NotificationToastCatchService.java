...
public class NotificationToastCatchService extends AccessibilityService {
	...
	public void onAccessibilityEvent(AccessibilityEvent event) {
		...
		if (eventType == AccessibilityEvent.TYPE_NOTIFICATION_STATE_CHANGED) {
			...
			if(parcelable!=null)
			{
				Log.i("DEBUG",parcelable.toString());
			}

			if ((parcelable instanceof Notification) || (parcelable instanceof NotificationCompat)) {
				...
				if (messages.size() > 0) {
					...
					Log.e(TAG, "Notification Message is empty. \nBTW, it's a notification");
				}
				NotificationToastCatchService.this.getApplicationContext().sendBroadcast(mIntent);
			} else {
